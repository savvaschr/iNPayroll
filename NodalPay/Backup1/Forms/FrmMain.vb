        ...
        If Global1.IsConnected Then
            ...
            If Not U Is Nothing Then
                If U.IsSA = "Y" Then
                    ...
                    Me.mnuSystemUpgrade.Visible = True
                    Me.mnuSystemUpgrade.Enabled = True
                    'Me.TestToolStripMenuItem3.Visible = True
                End If
            End If
            '   Me.mnuSystemUpgrade.Visible = True
            '  Me.mnuSystemUpgrade.Enabled = True
    ...
    Private Sub InitializeMe()
        ...
        For i = 0 To SDBArray.Length - 1
            If InStr(SDBArray(i), SDBDelimiter) = 0 Then
                MsgBox("Delimiter ';' betweeen Server And DataBase is missing." & _
    ...
    Private Sub LoadDataFromHCMsystem(ByVal OnlyNewEmployees As Boolean)
        ...
        Try
            ...
            If pCn.State = ConnectionState.Open Then
                ...
                " MobilePhone," & _
                " Email, " & _
                " JobDescriptionCode," & _
                " EmployeeJobDescription," & _
                " PayrollCompanyNo  " & _
                " FROM Employee"
    ...
    Private Sub Import_From_Excel_JIRA_1(ByVal TemplateGroupForLoading As String)
        ...
        Try
            ...
            Do While StopInput = False
                ...
                Period = NothingToEmpty(xlWorkSheet.Cells(Counter, 8).value)
                Account_Key = NothingToEmpty(xlWorkSheet.Cells(Counter, 9).value)
                Account_Name = NothingToEmpty(xlWorkSheet.Cells(Counter, 10).value)
                Account_Lead_ID = NothingToEmpty(xlWorkSheet.Cells(Counter, 11).value)
                Account_Category = NothingToEmpty(xlWorkSheet.Cells(Counter, 12).value)
                Account_Customer = NothingToEmpty(xlWorkSheet.Cells(Counter, 13).value)
                Activity_Name = NothingToEmpty(xlWorkSheet.Cells(Counter, 14).value)
                Component = NothingToEmpty(xlWorkSheet.Cells(Counter, 15).value)
                All_Components = NothingToEmpty(xlWorkSheet.Cells(Counter, 16).value)
                Version_Name = NothingToEmpty(xlWorkSheet.Cells(Counter, 17).value)
                Issue_Type = NothingToEmpty(xlWorkSheet.Cells(Counter, 18).value)
                Issue_Status = NothingToEmpty(xlWorkSheet.Cells(Counter, 19).value)
                Project_Key = NothingToEmpty(xlWorkSheet.Cells(Counter, 20).value)
                Project_Name = NothingToEmpty(xlWorkSheet.Cells(Counter, 21).value)
                Epic = NothingToEmpty(xlWorkSheet.Cells(Counter, 22).value)
                Epic_Link = NothingToEmpty(xlWorkSheet.Cells(Counter, 23).value)
                Work_Description = NothingToEmpty(xlWorkSheet.Cells(Counter, 24).value)
                Parent_Key = NothingToEmpty(xlWorkSheet.Cells(Counter, 25).value)
                Reporter_ID = NothingToEmpty(xlWorkSheet.Cells(Counter, 26).value)
                External_Hours = NothingToEmpty(xlWorkSheet.Cells(Counter, 27).value)
                Billed_Hours = NothingToEmpty(xlWorkSheet.Cells(Counter, 28).value)
                Issue_Original_Estimate = NothingToEmpty(xlWorkSheet.Cells(Counter, 29).value)
                Issue_Remaining_Estimate = NothingToEmpty(xlWorkSheet.Cells(Counter, 30).value)
                Date_created = NothingToEmpty(xlWorkSheet.Cells(Counter, 31).value)
                Date_updated = NothingToEmpty(xlWorkSheet.Cells(Counter, 32).value)
                ...
                With Ta
                    ...
                    Dim Anl5 As New cPrAnEmployeeAnalysis5(Project_Key)
                    If Anl5.EmpAn5_Code = "" Or Anl5.EmpAn5_Code Is Nothing Then
                        Anl5.EmpAn5_Code = Project_Key
                        If Project_Name.Length > 40 Then
                            Project_Name = Project_Name.Substring(0, 39)
                        End If
                        Anl5.EmpAn5_DescriptionL = Project_Name
                        If Project_Name.Length > 14 Then
                            DescS = Project_Name.Substring(0, 13)
                        Else
                            DescS = Project_Name
                        End If
                        Anl5.EmpAn5_DescriptionS = Project_Name
                        Anl5.EmpAn5_IsActive = "Y"
                        Anl5.GLAnal1 = Project_Key
                        Anl5.GLAnal2 = "8|0"
                        ...
                        End If
                    End If
                    .AnalCode = Project_Key
                    .AnalDesc = Project_Name
