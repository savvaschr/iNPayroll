Public Class FrmPrSsDecreaseTable
    Dim tPrSsdecreaseTable As New cPrSsDecreaseTable
    Dim DG1Changing As Boolean = False
    Private Sub frmPrSsDecreaseTable_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Me.Top = 0
        Me.Left = 0
        Initialize()
    ...
    Private Function ValidateMe() As Boolean
        ...
        '            Dim tPrSsTaxTable As New cprssextrataxtable(CInt(Trim(Me.txtTaxTbl_id.Text)))
    ...
    Private Sub ClearMe()
        ...
        Me.txtTaxTbl_DedRate.Text = "0.00"
        Me.txtTaxTbl_Code.Text = ""
    ...
    Private Sub PutDecimalValidationOnTxts()
        ...
        AddHandler txtTaxTbl_DedRate.KeyPress, AddressOf NumericKeyPress
        AddHandler txtTaxTbl_DedRate.Leave, AddressOf NumericOnLeave
        'AddHandler txtTaxTbl_Code.KeyPress, AddressOf NumericKeyPress
        'AddHandler txtTaxTbl_Code.Leave, AddressOf NumericOnLeave
    End Sub
    '
    Private Sub ClearErrors()
        ...
        Me.ErrTaxTbl_BracketRate.SetError(Me.txtTaxTbl_DedRate, "")
    ...
    Private Sub TSBNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TSBNew.Click
        ...
        tPrSsdecreaseTable = New cPrSsDecreaseTable
        ClearMe()
        ClearErrors()
    ...
    Private Sub TryToSave()
        If ValidateMe() Then
            ...
            Try
                ...
                '       Update = False
                '  End If
                With tPrSsdecreaseTable
                    .DecTbl_id = NullToInt(Me.txtTaxTbl_id.Text)
                    .DecTbl_Sequence = NullToInt(Me.txtTaxTbl_Sequence.Text)
                    .DecTbl_BracketAmount = CDbl(Me.txtTaxTbl_BracketAmount.Text)
                    .DecTbl_DedRate = CDbl(Me.txtTaxTbl_DedRate.Text)
                    .DecTbl_Code = Me.txtTaxTbl_Code.Text
                    .DecTbl_CreationDate = Me.DateCreated.Value.Date
                    .DecTbl_CreatedBy = CType(Me.cmbTaxTbl_CreatedBy.SelectedItem, cAaSsUsers).Id
                    .DecTbl_AmendDate = Me.DateAmend.Value.Date
                    .DecTbl_AmendBy = CType(Me.cmbTaxTbl_AmendBy.SelectedItem, cAaSsUsers).Id
                    If .Save() Then
    ...
    Private Sub LoadPrSsTaxTable(ByVal tTaxTbl_id As Integer)
        tPrSsdecreaseTable = New cPrSsDecreaseTable(tTaxTbl_id)
        If tPrSsdecreaseTable.DecTbl_id <> 0 Then
            With tPrSsdecreaseTable
                Me.txtTaxTbl_id.ReadOnly = True
                Me.txtTaxTbl_id.BackColor = SystemColors.Info
                Me.txtTaxTbl_id.Text = CStr(.DecTbl_id)
                Me.txtTaxTbl_Sequence.Text = CStr(.DecTbl_Sequence)
                Me.txtTaxTbl_BracketAmount.Text = Format(.DecTbl_BracketAmount, "0.00")
                Me.txtTaxTbl_DedRate.Text = Format(.DecTbl_DedRate, "0.00")
                Me.txtTaxTbl_Code.Text = .DecTbl_Code
                Me.DateCreated.Value = CDate(.DecTbl_CreationDate)
                ' Need to decide what to do with a combo in the load sub Property = TaxTbl_CreatedBy
                Me.DateAmend.Value = CDate(.DecTbl_AmendDate)
    ...
    Private Sub FillDG1()
        Dim ds As DataSet
        ds = Global1.Business.AG_GetAllPrSsDecreaseTable()
        DG1Changing = True
    ...
    Private Sub TSBDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles TSBDelete.Click
        ...
        If Response = 1 Then
            If tPrSsdecreaseTable.Delete(CInt(Trim(Me.txtTaxTbl_id.Text))) Then
    ...
    Private Sub LoadDataFromDG1(ByVal i As Integer)
        ...
        If Me.DG1.RowCount > 0 Then
            ...
            Me.txtTaxTbl_Code.Text = DbNullToString(DG1.Item(2, i).Value)
            Me.txtTaxTbl_BracketAmount.Text = DbNullToString(DG1.Item(3, i).Value)
            Me.txtTaxTbl_DedRate.Text = DbNullToString(DG1.Item(4, i).Value)
            Me.DateCreated.Value = DbNullToDate(DG1.Item(5, i).Value)
            Dim U1 As New cUsers(DbNullToInt(DG1.Item(6, i).Value))
            ...
            Dim U2 As New cUsers(DbNullToInt(DG1.Item(8, i).Value))
