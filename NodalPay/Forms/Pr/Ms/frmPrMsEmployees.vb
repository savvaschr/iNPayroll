...
Public Class frmPrMsEmployees
    ...
    Private Sub CheckPermition()
        ...
        If PARAM_Exelsys Then
            TSExelsys.Visible = True
        Else
            TSExelsys.Visible = False
        End If
        If UCase(Global1.UserName) = "SA" Or UCase(Global1.UserName) = "NODAL" Or UCase(Global1.UserName) = "INSOFT" Then
            'skip
        Else

            Ds = Global1.Business.GetParameter("System", "LockExelsys")
            If CheckDataSet(Ds) Then
                Dim Par As New cPrSsParameters(Ds.Tables(0).Rows(0))
                If Par.Value1 = 1 Then
                    LockFieldsForExelsys()
                    GLBLockForExelsys = True
                End If
            End If
        End If
    ...
    Private Sub ClearMe()
        ...
        Try
            Me.ComboSocialIns.SelectedIndex = 0
        Catch ex As Exception
        End Try
    ...
    Private Sub PutDecimalValidationOnTxts()
        ...
        AddHandler txtEmp_PrevUnion.KeyPress, AddressOf NumericKeyPress
        AddHandler txtEmp_PrevUnion.Leave, AddressOf NumericOnLeave
    ...
    Private Sub TryToSave(Optional ByVal ShowMsg As Boolean = True)
        If ValidateMe() Then
            ...
            Dim S As String
            Dim Ar() As String
            Me.txtCode.Text = Trim(Me.txtCode.Text)
            Try
                ...
                With tPrMsEmployees
                    .Code = CStr(Trim(Me.txtCode.Text))
                    ...
                    .PreviousUnion = CDbl(Me.txtEmp_PrevUnion.Text)
                    ...
                    Dim EmpPho As New cPrMsEmployeePhoto
                    EmpPho.Code = tPrMsEmployees.Code
                    EmpPho.MyPhoto = EmpPhoto.Image
    ...
    Private Sub LoadDataSetToExcel()
        ...
        HeaderStr.Add("Interface Template")
    ...
    Public Sub LoadEmployee(ByVal Emp As cPrMsEmployees, ByVal FromCopy As Boolean)
        ...
        With Emp
            ...
            ' If Me.cmbEmpAn1_Code.Text = "" Then
            ' Dim An1 As New cPrAnEmployeeAnalysis1(.EmpAn1_Code)
            'Me.cmbEmpAn1_Code.Text = An1.Code & " - " & An1.DescriptionL
            'End If
            ...
            Me.txtEmp_Prevunion.Text = .PreviousUnion
            ...
            Dim EmpPho As New cPrMsEmployeePhoto(.Code)
            If EmpPho.Code <> "" Then
                Me.EmpPhoto.Image = EmpPho.MyPhoto
            End If
        ...
        If TSBSalary.Enabled Then
            If Me.txtCode.Text <> "" Then
                ...
                Else
                    ...
                    F.GLBLockForExcelsys = GLBLockForExelsys
    ...
    Private Sub TSBDiscounts_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TSBDiscounts.Click
        If Me.txtCode.Text <> "" Then
            Dim tEmp As New cPrMsEmployees(Trim(Me.txtCode.Text))
            If tEmp.Code = "" Then
                MsgBox("Please save First Employee and then Proceed to Employee Discount", MsgBoxStyle.Information)
                Exit Sub
            Else
        ...
        End If
    End Sub
    Private Sub mnuExtraDetailsEmployeeExtraDetailsToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles mnuExtraDetailsEmployeeExtraDetailsToolStripMenuItem.Click
        If Me.txtCode.Text <> "" Then
            Dim tEmp As New cPrMsEmployees(Trim(Me.txtCode.Text))
            If tEmp.Code = "" Then
                MsgBox("Please save First Employee and then Proceed to Employee Extra Details", MsgBoxStyle.Information)
                Exit Sub
            Else
                Dim F As New frmPrMsEmployeeExtraDetails
                F.EmpCode = Me.txtCode.Text
                F.EmpName = Me.txtFullName.Text
                F.Emp = Me.GlbEmp
                ' F.TempGrpCode = tEmp.TemGrp_Code
    ...
    Private Sub TSBAnnualAdvances_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TSBAdvances.Click
        If Me.txtCode.Text <> "" Then
            ...
            Dim tEmp As New cPrMsEmployees(Trim(Me.txtCode.Text))
            If tEmp.Code = "" Then
                MsgBox("Please save First Employee and then Proceed to Employee Annual Leave", MsgBoxStyle.Information)
                Exit Sub
            Else
    ...
    Private Sub cmbPayUni_Code_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbPayUni_Code.SelectedIndexChanged
        If Me.cmbPayUni_Code.Text <> "" Then
            ...
            If PayUni.Code = 1 Then
                ...
                If Me.txtPeriodUnits.Text <> "" Then
                    If CDbl(Me.txtPeriodUnits.Text) <> 0 Then
...
#End Region
    ...
    Public Sub PreviousEmployee_OnSalary(ByVal F As frmPrTxEmployeeSalary)
        ...
        F.LoadMe()
    End Sub
    Public Sub NextEmployee_OnExtraDetails(ByVal F As frmPrMsEmployeeExtraDetails)
        GetEmployee(Me.txtCode.Text, True, False)
    ...
    End Sub
    Public Sub PreviousEmployee_OnExtraDetails(ByVal F As frmPrMsEmployeeExtraDetails)
        GetEmployee(Me.txtCode.Text, False, False)
    ...
    Private Sub GetEmployee(ByVal Code As String, ByVal NextEmp As Boolean, ByVal SameCode As Boolean, Optional OnlyThisTemplateGroup As String = "")

        Dim ds As DataSet
        Dim OnlyActive As Boolean = False
        If CBSearchOnlyActive.CheckState = CheckState.Checked Then
            OnlyActive = True
        End If
        ds = Global1.Business.FindEmployeeOfUser(Code, NextEmp, Global1.UserName, SameCode, OnlyActive, OnlyThisTemplateGroup)
        If CheckDataSet(ds) Then
    ...
    Private Sub UpdateEmployeesAfterEDCAddition(TemplateOnly As Boolean)
        Dim CurrentTemplateGroup As String = ""
        Dim EmpCode As String
        CurrentTemplateGroup = CType(Me.cmbTemGrp_Code.SelectedItem, cPrMsTemplateGroup).Code
        If TemplateOnly = False Then
            GetEmployee(Me.txtCode.Text, True, False)
        ...
        Do While txtCode.Text <> ""
            ...
            GetEmployee(Me.txtCode.Text, True, False, CurrentTemplateGroup)
    ...
    Private Function GetDetailsFromPrevious(ByVal Code As String) As cPrMsEmployees
        ...
        TempGrpCode = CType(Me.cmbTemGrp_Code.SelectedItem, cPrMsTemplateGroup).Code

        ds = Global1.Business.FindEmployeeOfUser(Code, False, Global1.UserName, False, True, TempGrpCode)
        If CheckDataSet(ds) Then
    ...
    Private Sub ChangeCodeToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ChangeCodeToolStripMenuItem.Click
        If GlbEmp.Code <> "" Then
            ...
            F.GlbTemplateGroupCode = GlbEmp.TemGrp_Code
            F.Owner = Me
            F.ShowDialog()
        Else
            MsgBox("Please select valid Employee", MsgBoxStyle.Critical)
        End If
    ...
    Private Sub btnCreateEmployees_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCreateEmployees.Click
        Try
            ...
            For i = 0 To 10
                NewClick()
                Me.txtCode.Text = 1 & (1 + i).ToString.PadLeft(3, "0")
                Me.txtTitle.Text = "Mr"
                Me.txtFirstName.Text = "Employee"
                Me.txtLastName.Text = i + 1
                Me.DateStart.Value = CDate("2024-01-01")
                Me.txtComSin_EmpSocialInsNo.Text = "2222222/2/2222"
                Me.txtBankAccountCo.Text = "99999999999"
                Me.txtBankAccount.Text = (i + 1).ToString.PadLeft(2, "0") & "99887766"
                Me.txtPayslipreport.Text = "PayWithLogo1.rpt"
                ...
                With EmpSal
                    ...
                    .Date1 = Now
                    .SalaryValue = 1000 + (i * 50)
                    .Basic = 0
                    .EffPayDate = CDate("2014-01-01")
                    .Cola = 0
                    .EffArrearsDate = CDate("2014-01-01")
    ...
    Private Sub UpdateEmployeesAfterEDCAdditionToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles UpdateEmployeesAfterEDCAdditionToolStripMenuItem.Click
        UpdateEmployeesAfterEDCAddition(False)
    End Sub
    Private Sub UpdateEmployeesAfterEDCAddition_TempOnly_TSM_Click(sender As Object, e As EventArgs) Handles UpdateEmployeesAfterEDCAddition_TempOnly_TSM.Click
        UpdateEmployeesAfterEDCAddition(True)
    End Sub
    ...
    Private Sub txtPeriodUnits_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtPeriodUnits.TextChanged
        If Me.txtPeriodUnits.Text <> "" Then
            If CDbl(Me.txtPeriodUnits.Text) <> 0 Then
                Me.txtAnnualUnits.ReadOnly = False
            End If
            If CDbl(Me.txtPeriodUnits.Text) = 0 Then
                Me.txtAnnualUnits.ReadOnly = True
                Me.txtAnnualUnits.Text = "0.00"
            End If
        End If
    End Sub

    Private Sub RemoveFirstEmploymentDiscountFromActiveEmployeesToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RemoveFirstEmploymentDiscountFromActiveEmployeesToolStripMenuItem.Click
        Dim Emp As New cPrMsEmployees(Me.txtCode.Text)
        If Emp.Code <> "" Then
            Dim Ans As New MsgBoxResult
            Ans = MsgBox("With this action First Employement flag will be removed from all Active Employees , continue ?", MsgBoxStyle.YesNo)
            If Ans = MsgBoxResult.Yes Then
                If Global1.Business.RemoveFirstEmploymentFromActiveEmployees(Emp.TemGrp_Code) Then
                    MsgBox("First Employement Removed from Active Employees", MsgBoxStyle.Information)
                Else
                    MsgBox("Error when trying to Remove First Employment Flag from Active Employees", MsgBoxStyle.Critical)
                End If
            End If
        Else
    ...
    Private Sub TSExelsysLoad_Click(sender As Object, e As EventArgs) Handles TSExelsysLoad.Click
    ...
    Private Sub TSExelsysAuditReport_Click(sender As Object, e As EventArgs) Handles TSExelsysAuditReport.Click
        Dim F As New FrmExelsysAuditReport
        F.Owner = Me
        F.Show()
    End Sub
    Private Sub LockFieldsForExelsys()
        ...
        Me.ComboHireReason.Enabled = False
        Me.ComboTermReason.Enabled = False
        Me.cmbEmpSta_Code.Enabled = False
        ...
        Me.cmbMarSta_Code.Enabled = False
        ...
        Me.txtTelephone1.ReadOnly = True
        Me.txtTelephone2.ReadOnly = True
        Me.txtEmail.ReadOnly = True
        Me.txtEmail2.ReadOnly = True
        Me.txtSocialInsNumber.ReadOnly = True
        Me.txtComSin_EmpSocialInsNo.ReadOnly = True
        Me.txtIdentificationCard.ReadOnly = True
        Me.txtTaxId.ReadOnly = True
        Me.txtPassportNumber.ReadOnly = True
        Me.txtAlienNumber.ReadOnly = True
        Me.txtBankBenName.ReadOnly = True

        Me.cmbTaxCardType.Enabled = False
        Me.cmbEmpAn1_Code.Enabled = False
        Me.cmbEmpAn2_Code.Enabled = False
        Me.cmbEmpAn3_Code.Enabled = False
        Me.cmbEmpAn4_Code.Enabled = False
        Me.cmbEmpAn5_Code.Enabled = False
        Me.cmbUni_Code.Enabled = False
        Me.cmbCou_Code.Enabled = False
        Me.cmbEmpPos_Code.Enabled = False
        Me.cmbSic_Code.Enabled = False
        Me.cmbEmpCmm_Code.Enabled = False
        Me.cmbPayUni_Code.Enabled = False
        Me.ComboProFund.Enabled = False
        Me.ComboMedicalFund.Enabled = False
        Me.ComboGESI.Enabled = False
        Me.ComboIndustrial.Enabled = False
        Me.ComboUnemployment.Enabled = False
        Me.ComboSocialCohesion.Enabled = False


        Me.cmbCur_Code.Enabled = False
        Me.cmbPmtMth_Code.Enabled = False
        Me.cmbBnk_Code.Enabled = False
        Me.txtBankAccount.ReadOnly = True
        Me.cmbBnk_CodeCo.Enabled = False
        Me.ComboSectorPay.Enabled = False
        Me.ComboCommissionRates.Enabled = False
        Me.ComboPerformanceBonus.Enabled = False
        Me.ComboDutyHours.Enabled = False
        Me.ComboOverLay.Enabled = False
        Me.ComboFlightHours.Enabled = False
        Me.ComboSocialIns.Enabled = False
        Me.txtBankAccountCo.ReadOnly = True
        Me.DateStart.Enabled = False
        Me.txtTerminateDate.ReadOnly = True

        'Me.txtOtherIncome1.Text = "0.00"
        'Me.txtOtherIncome2.Text = "0.00"
        'Me.txtOtherIncome3.Text = "0.00"

        Me.txtAgreedSalary.ReadOnly = True
        Me.txtExtraBonusOnSalary.ReadOnly = True
        Me.txtTaxForSplit.ReadOnly = True
        ...
        Me.txtPayslipreport.ReadOnly = True
        Me.txtEmployeeIBAN.ReadOnly = True
        ...
        Me.txtFullPassportName.ReadOnly = True
        Me.txtTravelDocs.ReadOnly = True
        'Me.CBFirstEmployment.CheckState = CheckState.Unchecked
        'Me.txtFEControlAmount.Text = "0.00"
        'Me.CBForce50Percent.CheckState = CheckState.Unchecked
        'Me.CBLimitTo20.CheckState = CheckState.Unchecked
        ...
        Me.txtBankBenName.ReadOnly = True
        Me.txtPensionNo.ReadOnly = True

        'Me.txtPrevGesiD.Text = "0.00"
        'Me.txtPrevGesiC.Text = "0.00"
        Me.txtNameDay.ReadOnly = True
        Me.txtAnalGen1.ReadOnly = True

        txtAnnualGesyFromSplit.ReadOnly = True

        Me.txtGLAnal1.ReadOnly = True
        Me.txtGLAnal2.ReadOnly = True
        Me.txtGLAnal3.ReadOnly = True
        Me.txtGLAnal4.ReadOnly = True

        Me.txtOhterCountryTIC.ReadOnly = True
        ...
        Me.txtOtherIncome1.ReadOnly = True
        Me.txtOtherIncome4.ReadOnly = True


    End Sub

    Private Sub btnFixAnalysis1_Click(sender As Object, e As EventArgs) Handles btnFixAnalysis1.Click
        Dim ans As New MsgBoxResult
        If GlbEmp.Code <> "" Then
            Dim NewCode As String
            NewCode = CType(Me.cmbEmpAn1_Code.SelectedItem, cPrAnEmployeeAnalysis1).Code
            ans = MsgBox("Do you want to change Employee Analysis 1 value to " & NewCode & " on the last Payslip for then current employee ", MsgBoxStyle.YesNo)
            If ans = MsgBoxResult.Yes Then
                Dim Lastheaderid As Integer
                Lastheaderid = Global1.Business.FindLastPayslipOfEmployee(GlbEmp.Code)
                If Lastheaderid <> 0 Then

                    Global1.Business.UpdateTrxnHeaderAnalysis(NewCode, Lastheaderid, 1)
                    MsgBox("Employee Analysis 1 has changed", MsgBoxStyle.Information)
                Else
                    MsgBox("There are no Payroll transactions for this employee", MsgBoxStyle.Information)
                End If
            End If
        Else
            MsgBox("Please select valid Employee first", MsgBoxStyle.Information)
        End If

    End Sub
    Private Sub btnFixAnalysis2_Click(sender As Object, e As EventArgs) Handles btnFixAnalysis2.Click
        Dim ans As New MsgBoxResult
        If GlbEmp.Code <> "" Then
            Dim NewCode As String
            NewCode = CType(Me.cmbEmpAn2_Code.SelectedItem, cPrAnEmployeeAnalysis2).Code
            ans = MsgBox("Do you want to change Employee Analysis 2 value to " & NewCode & " on the last Payslip for then current employee ", MsgBoxStyle.YesNo)
            If ans = MsgBoxResult.Yes Then
                Dim Lastheaderid As Integer
                Lastheaderid = Global1.Business.FindLastPayslipOfEmployee(GlbEmp.Code)
                If Lastheaderid <> 0 Then

                    Global1.Business.UpdateTrxnHeaderAnalysis(NewCode, Lastheaderid, 2)
                    MsgBox("Employee Analysis 2 has changed", MsgBoxStyle.Information)
                Else
                    MsgBox("There are no Payroll transactions for this employee", MsgBoxStyle.Information)
                End If
            End If
        ...
        End If
    End Sub
    Private Sub btnFixAnalysis3_Click(sender As Object, e As EventArgs) Handles btnFixAnalysis3.Click
        Dim ans As New MsgBoxResult
        If GlbEmp.Code <> "" Then
            Dim NewCode As String
            NewCode = CType(Me.cmbEmpAn3_Code.SelectedItem, cPrAnEmployeeAnalysis3).Code
            ans = MsgBox("Do you want to change Employee Analysis 3 value to " & NewCode & " on the last Payslip for then current employee ", MsgBoxStyle.YesNo)
            If ans = MsgBoxResult.Yes Then
                Dim Lastheaderid As Integer
                Lastheaderid = Global1.Business.FindLastPayslipOfEmployee(GlbEmp.Code)
                If Lastheaderid <> 0 Then

                    Global1.Business.UpdateTrxnHeaderAnalysis(NewCode, Lastheaderid, 3)
                    MsgBox("Employee Analysis 3 has changed", MsgBoxStyle.Information)
                Else
                    MsgBox("There are no Payroll transactions for this employee", MsgBoxStyle.Information)
                End If
            End If
        ...
        End If
    End Sub
    Private Sub btnFixAnalysis4_Click(sender As Object, e As EventArgs) Handles btnFixAnalysis4.Click
        Dim ans As New MsgBoxResult
        If GlbEmp.Code <> "" Then
            Dim NewCode As String
            NewCode = CType(Me.cmbEmpAn4_Code.SelectedItem, cPrAnEmployeeAnalysis4).Code
            ans = MsgBox("Do you want to change Employee Analysis 4 value to " & NewCode & " on the last Payslip for then current employee ", MsgBoxStyle.YesNo)
            If ans = MsgBoxResult.Yes Then
                Dim Lastheaderid As Integer
                Lastheaderid = Global1.Business.FindLastPayslipOfEmployee(GlbEmp.Code)
                If Lastheaderid <> 0 Then

                    Global1.Business.UpdateTrxnHeaderAnalysis(NewCode, Lastheaderid, 4)
                    MsgBox("Employee Analysis 4 has changed", MsgBoxStyle.Information)
                Else
                    MsgBox("There are no Payroll transactions for this employee", MsgBoxStyle.Information)
                End If
            End If
        ...
        End If
    End Sub
    Private Sub btnFixAnalysis5_Click(sender As Object, e As EventArgs) Handles btnFixAnalysis5.Click
        Dim ans As New MsgBoxResult
        If GlbEmp.Code <> "" Then
            Dim NewCode As String
            NewCode = CType(Me.cmbEmpAn5_Code.SelectedItem, cPrAnEmployeeAnalysis5).EmpAn5_Code
            ans = MsgBox("Do you want to change Employee Analysis 5 value to " & NewCode & " on the last Payslip for then current employee ", MsgBoxStyle.YesNo)
            If ans = MsgBoxResult.Yes Then
                Dim Lastheaderid As Integer
                Lastheaderid = Global1.Business.FindLastPayslipOfEmployee(GlbEmp.Code)
                If Lastheaderid <> 0 Then

                    Global1.Business.UpdateTrxnHeaderAnalysis(NewCode, Lastheaderid, 5)
                    MsgBox("Employee Analysis 1 has changed", MsgBoxStyle.Information)
                Else
                    MsgBox("There are no Payroll transactions for this employee", MsgBoxStyle.Information)
                End If
    ...
    Private Sub btnFixUnion_Click(sender As Object, e As EventArgs) Handles btnFixUnion.Click
        Dim ans As New MsgBoxResult
        If GlbEmp.Code <> "" Then
            Dim NewCode As String
            NewCode = CType(Me.cmbUni_Code.SelectedItem, cPrAnUnions).Code
            ans = MsgBox("Do you want to change Employee Union value to " & NewCode & " on the last Payslip for then current employee ", MsgBoxStyle.YesNo)
            If ans = MsgBoxResult.Yes Then
                Dim Lastheaderid As Integer
                Lastheaderid = Global1.Business.FindLastPayslipOfEmployee(GlbEmp.Code)
                If Lastheaderid <> 0 Then

                    Global1.Business.UpdateTrxnHeaderAnalysis(NewCode, Lastheaderid, 6)
                    MsgBox("Employee Union has changed", MsgBoxStyle.Information)
                Else
                    MsgBox("There are no Payroll transactions for this employee", MsgBoxStyle.Information)
                End If
            End If
