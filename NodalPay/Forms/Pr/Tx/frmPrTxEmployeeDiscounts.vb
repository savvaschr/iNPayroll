Public Class frmPrTxEmployeeDiscounts
    ...
    Private Sub RemoToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles RemoToolStripMenuItem.Click
        DeleteDiscountsOfPeriodGroup()
    End Sub

    Private Sub DeleteDiscountsOfPeriodGroup()
        Dim Ans As New MsgBoxResult
        Dim PrdGrpCode As String
        PrdGrpCode = CType(Me.cmbPrdGrp_Code.SelectedItem, cPrMsPeriodGroups).Code
        Ans = MsgBox("With this Action Discounts of ALL employees of Period Group " & PrdGrpCode & " Will be deleted, Proceed ?", vbYesNo)
        If Ans = MsgBoxResult.Yes Then
            Ans = MsgBox("Are you Sure ?", vbYesNo)
            If Ans = MsgBoxResult.Yes Then
                If Global1.Business.DeleteAllEmployeeDiscountsOfPeriodGroup(PrdGrpCode) Then
                    MsgBox("Done")
                Else
        ...
        End If
    End Sub
    Private Sub RemoveFirstEmployementFromALLActiveEmployeesToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles RemoveFirstEmployementFromALLActiveEmployeesToolStripMenuItem.Click
        RemoveFirstEmployementOfActiveEmployees()
    End Sub
    Private Sub RemoveFirstEmployementOfActiveEmployees()
        Dim Ans As New MsgBoxResult
        Dim PrdGrpCode As String
        PrdGrpCode = CType(Me.cmbPrdGrp_Code.SelectedItem, cPrMsPeriodGroups).Code
        Dim PerGrp As New cPrMsPeriodGroups(PrdGrpCode)
        Dim TempGrp As New cPrMsTemplateGroup(PerGrp.TemGrpCode)
        Ans = MsgBox("With this Action First employment will be Removed from ALL Active Employees of Template Group " & TempGrp.Code & "-" & TempGrp.DescriptionL & " , Proceed ?", vbYesNo)
        If Ans = MsgBoxResult.Yes Then
            Ans = MsgBox("Are you Sure ?", vbYesNo)
            If Ans = MsgBoxResult.Yes Then
                If Global1.Business.RemoveFirstEmploymentFromAllActiveEmployees(TempGrp.Code) Then
                    MsgBox("Done")
                Else
                    MsgBox("Unable to Delete")
                End If
